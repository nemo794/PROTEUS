...
def build_product_id_str(granule_dir_path, granule_id, list_of_urls):
    '''
    Per the DSWx-HLS Product Spec (July 22, 2022), the desired Format of final product name:
    Project_Level_ProductType_Source_Sensor_PixelSpacing_TileID_AcquisitionDateTime_
    ProductGenerationDateTime_ProductVersion_LayerNumber_LayerName.ext
    Example: 
    OPERA_DSWx-HLS_S2A_30_T15SXR_20210205T163901Z_20220101T140222Z_v1.0_B01_WTR.tiff

    PROTEUS will handle creating and appending the '_v1.0_B01_WTR.tiff' substring. This function generates
    ...
    AcquisitionDateTime: The acquisition date and time (Greenwhich Meant Time or GMT) of the input satellite imagery for this product (format: YYYYMMDDTHHMMSSZ)
    ...
    # Close the file
    # (Use del, per: https://gis.stackexchange.com/questions/80366/why-close-a-dataset-in-gdal-python)
    del f
    ...
    if satellite == "S30":
        ...
        warnings.warn(f"Granule ID {granule_id}'s name could not be parsed to determine the Satellite.")
        product_id = "_UNKNOWN"
